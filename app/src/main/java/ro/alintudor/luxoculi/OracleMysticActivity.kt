...
import ro.alintudor.luxoculi.core.OracleAccountMailer
import ro.alintudor.luxoculi.core.OracleApiClient
import ro.alintudor.luxoculi.core.OracleAuthStore
import ro.alintudor.luxoculi.core.OracleBootstrap
import ro.alintudor.luxoculi.core.OracleFirebaseMessagingService
import ro.alintudor.luxoculi.core.OracleKnowledgeSync
import ro.alintudor.luxoculi.core.OracleLoaderQuotes
import ro.alintudor.luxoculi.core.OracleLocalProcessor
import ro.alintudor.luxoculi.core.OracleRepository
import ro.alintudor.luxoculi.core.OracleSyncManager
import ro.alintudor.luxoculi.core.snapshot
import ro.alintudor.luxoculi.nativeui.*
...
class OracleMysticActivity : Activity() {
    ...
    private val tickerRefreshRunnable: Runnable = Runnable {
        ...
        if (hero != null) {
            val marketOpen = runCatching { ro.alintudor.luxoculi.core.OracleMarketCalendar.status().open }.getOrDefault(false)
            val maxAgeMs = if (marketOpen) 3L * 60L * 1000L else 30L * 60L * 1000L
            if (ro.alintudor.luxoculi.core.OracleTopTickersCache.isStale(this, maxAgeMs)) {
                Thread {
                    ro.alintudor.luxoculi.core.OracleTopTickersCache.refresh(this)
                    val refreshed = ro.alintudor.luxoculi.core.OracleTopTickersCache.cached(this)
    ...
    private fun topTickersToDisplay(items: List<ro.alintudor.luxoculi.core.OracleTopTickersCache.Item>) =
    ...
    private fun showGrowthLogDialog() {
        val lines = ro.alintudor.luxoculi.core.OracleGrowthLog.read(this, 400)
        ...
        android.app.AlertDialog.Builder(this)
            ...
            .setNeutralButton("Download") { _, _ ->
                val path = ro.alintudor.luxoculi.core.OracleGrowthLog.export(this)
    ...
    private fun showNetworkLogDialog() {
        val lines = ro.alintudor.luxoculi.core.OracleNetworkLog.read(300)
        val panel = Color.rgb(7, 14, 28)
        val scroll = ScrollView(this).apply { setBackgroundColor(panel) }
        scroll.addView(TextView(this).apply {
    ...
    private fun offerBiometricEnrollIfNeeded(store: OracleAuthStore, onDone: () -> Unit) {
        if (store.biometricEnabled() || store.biometricOffered() || !biometricAvailable() || isFinishing || isDestroyed) {
            ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "biometric-enroll: skipped (enabled=${store.biometricEnabled()}, offered=${store.biometricOffered()}, available=${biometricAvailable()}, finishing=$isFinishing, destroyed=$isDestroyed)")
            onDone(); return
        }
        store.setBiometricOffered(true)
        ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "biometric-enroll: showing dialog")
        ...
        runCatching {
            android.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("Enable") { _, _ -> store.setBiometricEnabled(true); ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "biometric-enroll: user enabled"); onDone() }
                .setNegativeButton("Not now") { _, _ -> ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "biometric-enroll: user declined"); onDone() }
                .setCancelable(false)
                .show()
        }.onFailure { ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "biometric-enroll: dialog.show() threw: ${it.javaClass.simpleName}: ${it.message}"); onDone() }
    }
    ...
    override fun onResume() {
        ...
        runCatching {
            if (ro.alintudor.luxoculi.core.OracleDailyLogoutReceiver.applyIfDue(this)) {
                currentModule = null
    ...
    private fun proceedPastAuth() {
        ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "proceedPastAuth: entered (already proceeding=$proceedingPastAuth)")
        if (proceedingPastAuth) return
        proceedingPastAuth = true
        if (android.os.Build.VERSION.SDK_INT >= 33 &&
            checkSelfPermission(android.Manifest.permission.POST_NOTIFICATIONS) != android.content.pm.PackageManager.PERMISSION_GRANTED) {
            ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "proceedPastAuth: requesting POST_NOTIFICATIONS")
            runCatching { requestPermissions(arrayOf(android.Manifest.permission.POST_NOTIFICATIONS), 357) }
                .onFailure { ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "proceedPastAuth: requestPermissions threw: ${it.javaClass.simpleName}: ${it.message}") }
        }
        runCatching {
            OracleBootstrap.ensure(repository)
            OracleKnowledgeSync.scheduleNextCheck(this)
            ro.alintudor.luxoculi.core.OracleAlertCheckReceiver.schedule(this)
            ro.alintudor.luxoculi.core.OracleGrowthScanReceiver.schedule(this)
            ro.alintudor.luxoculi.core.OracleDailyLogoutReceiver.schedule(this)
            // If today's full-universe scan hasn't run yet (fresh install, or
            // the phone was off overnight), start it now in the background.
            ro.alintudor.luxoculi.core.OracleGrowthScanReceiver.scanNowIfMissing(this)
            ...
            ro.alintudor.luxoculi.widget.OracleGrowthWidgetProvider.updateAll(this)
            ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "proceedPastAuth: showing boot loader")
            showBootLoader()
        }.onFailure { proceedingPastAuth = false; ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "proceedPastAuth: FAILED: ${it.javaClass.simpleName}: ${it.message}"); showFatalError("Lux Oculi failed to start", it) }
    }
    ...
    private fun showLogin(store: OracleAuthStore) {
        ...
        loginButton.setOnClickListener {
            ...
            ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "login: button tapped for user=$username")
            Thread {
                val result = OracleApiClient.login(username, password)
                ro.alintudor.luxoculi.core.OracleGrowthLog.log(this@OracleMysticActivity, "AUTH", "login: API call returned, success=${result.isSuccess}")
                runOnUiThread {
                    ro.alintudor.luxoculi.core.OracleGrowthLog.log(this@OracleMysticActivity, "AUTH", "login: on UI thread with result")
                    result.onSuccess { token ->
                        store.saveSession(username, token)
                        ro.alintudor.luxoculi.core.OracleGrowthLog.log(this@OracleMysticActivity, "AUTH", "login: session saved, registering push token")
                        OracleFirebaseMessagingService.registerCurrentToken(this@OracleMysticActivity)
                        ro.alintudor.luxoculi.core.OracleGrowthLog.log(this@OracleMysticActivity, "AUTH", "login: starting pullAll")
                        OracleSyncManager.pullAll(this@OracleMysticActivity, token) { pullSuccess ->
                            ro.alintudor.luxoculi.core.OracleGrowthLog.log(this@OracleMysticActivity, "AUTH", "login: pullAll onDone, success=$pullSuccess")
                            offerBiometricEnrollIfNeeded(store) {
                                ro.alintudor.luxoculi.core.OracleGrowthLog.log(this@OracleMysticActivity, "AUTH", "login: biometric-enroll step done, calling proceedPastAuth")
        ...
        card.addView(TextView(this).apply {
            ...
            setOnClickListener {
                ...
                store.clearSession()
                ro.alintudor.luxoculi.core.OracleDemo.enter(this@OracleMysticActivity)
                ro.alintudor.luxoculi.widget.OracleGrowthWidgetProvider.updateAll(this@OracleMysticActivity)
                authPassedThisProcess = true
                proceedPastAuth()
            }
        })
    ...
    private fun showHub() {
        ...
        runCatching {
            ...
            val cachedTop = ro.alintudor.luxoculi.core.OracleTopTickersCache.cached(this)
        ...
        runCatching {
            hero.setUnreadMessages(ro.alintudor.luxoculi.core.OracleInboxStore(this).unreadCount())
        }
        checkServerConnectionSilently(hero)
        runCatching {
            if (ro.alintudor.luxoculi.core.OracleInboxStore(this).unreadCount() > 0) showMessagesDialog()
        }
    }
    ...
    private fun checkServerConnectionSilently(hero: OracleMysticStartView) {
        if (ro.alintudor.luxoculi.core.OracleGrowthEmergency.isForcingLocal(this)) {
            ...
            Thread { ro.alintudor.luxoculi.core.OracleApiClient.ping() }.start()
            return
        }
        Thread {
            val ok = ro.alintudor.luxoculi.core.OracleApiClient.ping().isSuccess
            runOnUiThread {
    ...
    private fun confirmExitDemo() {
        android.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("Exit") { _, _ ->
                ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "exit-demo confirmed")
                val store = OracleAuthStore(this)
                ro.alintudor.luxoculi.core.OracleDemo.exit(this)
                store.clearSession()
                ro.alintudor.luxoculi.widget.OracleGrowthWidgetProvider.updateAll(this)
                ...
                root.post { ro.alintudor.luxoculi.core.OracleGrowthLog.log(this, "AUTH", "exit-demo: showing login screen (posted)"); showLogin(store) }
            }
    ...
    private fun showBackupScreen() {
        ...
        if (ro.alintudor.luxoculi.core.OracleDemo.active(this)) { confirmExitDemo(); return }
        root.removeAllViews()
        val bg = Color.rgb(3, 4, 12); val panel = Color.rgb(7, 14, 28); val border = Color.rgb(49, 82, 125)
            ...
            setOnClickListener {
                android.app.AlertDialog.Builder(this@OracleMysticActivity)
                    ...
                    .setPositiveButton("Log out") { _, _ ->
                        ...
                        store.clearSession()
                        ro.alintudor.luxoculi.core.OracleAdminAccess.lock()
                        ro.alintudor.luxoculi.widget.OracleGrowthWidgetProvider.updateAll(this@OracleMysticActivity)
                        authPassedThisProcess = false
                        currentModule = null
                        showLogin(store)
        ...
        if (ro.alintudor.luxoculi.core.OracleAdminAccess.isOwnerAccount(this)) {
    ...
    private fun showMessagesDialog() {
        ...
        val store = ro.alintudor.luxoculi.core.OracleInboxStore(this)
    ...
    private fun showUltraShortDialog() {
        val panel = Color.rgb(7, 14, 28); val muted = Color.rgb(165, 174, 195); val gold = Color.rgb(255, 205, 55)
        val store = ro.alintudor.luxoculi.core.OracleUltraShortJournalStore(this)
        ...
        if (today != null) {
            ...
            val names = ro.alintudor.luxoculi.nativeui.OracleFactorGrid.NAMES
    ...
    private fun showChangePinDialog() {
        ...
        android.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                ro.alintudor.luxoculi.core.OracleAdminAccess.setPin(this, pin)
    ...
    private fun promptAdminAccess() {
        if (ro.alintudor.luxoculi.core.OracleAdminAccess.isUnlockedThisProcess()) { showAdminScreen(); return }
        ...
        if (!ro.alintudor.luxoculi.core.OracleAdminAccess.hasPin(this)) {
            android.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("Set PIN") { _, _ ->
                    ...
                    if (pin.length < 4) { Toast.makeText(this, "Use at least 4 digits.", Toast.LENGTH_SHORT).show(); return@setPositiveButton }
                    ro.alintudor.luxoculi.core.OracleAdminAccess.setPin(this, pin)
                    ro.alintudor.luxoculi.core.OracleAdminAccess.markUnlocked()
                    showAdminScreen()
                }
                .setNegativeButton("Cancel", null).show()
            ...
            android.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("Unlock") { _, _ ->
                    if (ro.alintudor.luxoculi.core.OracleAdminAccess.verifyPin(this, pinField.text.toString().trim())) {
                        ro.alintudor.luxoculi.core.OracleAdminAccess.markUnlocked()
                        showAdminScreen()
            ...
            .setPositiveButton("Verify") { _, _ ->
                ...
                Thread {
                    ...
                    runOnUiThread {
                        if (isFinishing) return@runOnUiThread
                        result.onSuccess {
                            ro.alintudor.luxoculi.core.OracleAdminAccess.clearPin(this)
    ...
    private fun showAdminScreen() {
        ...
        val logCount = ro.alintudor.luxoculi.core.OracleGrowthLog.lineCount(this)
        card.addView(TextView(this).apply {
        ...
        logRow.addView(toolButton("CLEAR", Color.rgb(255, 140, 140)) {
            android.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("Clear") { _, _ ->
                    ro.alintudor.luxoculi.core.OracleGrowthLog.clear(this)
        ...
        val journalStore = ro.alintudor.luxoculi.core.OracleGrowthJournalStore(this)
        ...
        val netLogCount = ro.alintudor.luxoculi.core.OracleNetworkLog.lineCount()
        card.addView(TextView(this).apply {
        ...
        netLogRow.addView(toolButton("CLEAR", Color.rgb(255, 140, 140)) {
            ro.alintudor.luxoculi.core.OracleNetworkLog.clear()
        ...
        val emergencyLoaded = ro.alintudor.luxoculi.core.OracleGrowthEmergency.isLoaded(this)
        val emergencyLoadedAt = ro.alintudor.luxoculi.core.OracleGrowthEmergency.loadedAt(this)
        card.addView(TextView(this).apply {
        ...
        emergencyRow.addView(toolButton("CLEAR", Color.rgb(255, 140, 140)) {
            ...
            android.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("Clear") { _, _ ->
                    ro.alintudor.luxoculi.core.OracleGrowthEmergency.clear(this)
        ...
        val forcingLocal = ro.alintudor.luxoculi.core.OracleGrowthEmergency.isForcingLocal(this)
        card.addView(TextView(this).apply {
        ...
        card.addView(toolButton(if (forcingLocal) "TURN OFF — USE SERVER AGAIN" else "FORCE LOCAL MODE (TESTING)", if (forcingLocal) Color.rgb(255, 170, 40) else Color.rgb(55, 215, 255)) {
            ...
            ro.alintudor.luxoculi.core.OracleGrowthEmergency.setForceLocal(this, turningOn)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            ro.alintudor.luxoculi.nativeui.OraclePortfolioModule.CSV_IMPORT_REQUEST -> Thread {
                val text = runCatching { contentResolver.openInputStream(uri)?.bufferedReader(Charsets.UTF_8)?.use { it.readText() } ?: "" }.getOrDefault("")
                val rows = runCatching { ro.alintudor.luxoculi.core.OracleCsvImport.parse(text) }.getOrDefault(emptyList())
                val n = if (rows.isNotEmpty()) ro.alintudor.luxoculi.nativeui.OraclePortfolioModule.applyImport(this, rows) else 0
                runOnUiThread {
            ...
            EMERGENCY_IMPORT_REQUEST -> Thread {
                val text = runCatching { contentResolver.openInputStream(uri)?.bufferedReader(Charsets.UTF_8)?.use { it.readText() } ?: "" }.getOrDefault("")
                val summary = ro.alintudor.luxoculi.core.OracleGrowthEmergency.importFrom(this, text)
                runOnUiThread {
    ...
    private fun handleUnauthorizedIfNeeded(error: Throwable): Boolean {
        ...
        while (e != null) {
            if (e is ro.alintudor.luxoculi.core.OracleUnauthorizedException) {
                val store = OracleAuthStore(this)
                store.clearSession()
                ro.alintudor.luxoculi.core.OracleAdminAccess.lock()
                ro.alintudor.luxoculi.widget.OracleGrowthWidgetProvider.updateAll(this)
