...
import ro.alintudor.luxoculi.OracleMysticActivity
import ro.alintudor.luxoculi.R
import ro.alintudor.luxoculi.core.OracleGrowthRecommendation
...
class OracleGrowthWidgetProvider : AppWidgetProvider() {
    companion object {
        private const val ACTION_REFRESH = "ro.alintudor.luxoculi.widget.ACTION_REFRESH_GROWTH"
        ...
        fun updateAll(context: Context) {
            ...
            val authorized = runCatching {
                ro.alintudor.luxoculi.core.OracleAuthStore(appContext).hasSession() && !ro.alintudor.luxoculi.core.OracleDemo.active(appContext)
            }.getOrDefault(false)
        ...
        fun validateSessionAsync(context: Context) {
            val appContext = context.applicationContext
            val store = ro.alintudor.luxoculi.core.OracleAuthStore(appContext)
            if (!store.hasSession() || ro.alintudor.luxoculi.core.OracleDemo.active(appContext)) return
            val token = store.token()
            if (token.isBlank()) return
            var e: Throwable? = ro.alintudor.luxoculi.core.OracleApiClient.checkSession(token).exceptionOrNull()
            var revoked = false
            while (e != null) {
                if (e is ro.alintudor.luxoculi.core.OracleUnauthorizedException) { revoked = true; break }
                e = e.cause
            }
            if (revoked) {
                store.clearSession()
        ...
        private fun buildViews(context: Context, items: List<OracleGrowthRecommendation>): RemoteViews {
            ...
            val marketStatus = runCatching { ro.alintudor.luxoculi.core.OracleMarketCalendar.status() }.getOrNull()
