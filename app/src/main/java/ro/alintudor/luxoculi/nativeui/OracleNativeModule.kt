...
import ro.alintudor.luxoculi.core.OracleMarketCalendar
...
class OracleNativeModule(
    ...
    init {
        ...
        when (moduleKey) {
            "growth" -> {
                val local = ro.alintudor.luxoculi.core.OracleGrowthEmergency.isForcingLocal(context)
            ...
            "knowledge" -> {
                val hasError = ro.alintudor.luxoculi.core.OracleKnowledgeSync.lastError(context).isNotBlank()
        ...
        buildRow.addView(ImageView(context).apply { setImageResource(ro.alintudor.luxoculi.R.drawable.ic_oracle); scaleType=ImageView.ScaleType.CENTER_INSIDE }, LinearLayout.LayoutParams(dp(17),dp(17)).apply{ setMargins(0,dp(3),dp(5),0) })
        buildRow.addView(TextView(context).apply { text=ro.alintudor.luxoculi.core.OracleBuildInfo.label(title);textSize=10f;typeface=Typeface.DEFAULT_BOLD;letterSpacing=.10f;setTextColor(Color.rgb(25,205,255));gravity=Gravity.CENTER;includeFontPadding=true })
        ...
        if (ro.alintudor.luxoculi.core.OracleDemo.active(context)) {
            root.addView(TextView(context).apply {
                text = "DEMO  \u00b7  ${ro.alintudor.luxoculi.core.OracleDemo.LOCK} locked values \u2014 create an account for full access"
