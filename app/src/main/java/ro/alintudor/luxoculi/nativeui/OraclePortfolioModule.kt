...
import ro.alintudor.luxoculi.core.*
import java.io.File
...
class OraclePortfolioModule(private val host: OracleNativeModule) {
    ...
    private fun card(rank: Int, p: OraclePosition, a: OracleAction?, t: OracleTechnicalSnapshot?, journal: List<OracleJournalEntry>, silent: Boolean = false) {
        ...
        val tickerHead = LinearLayout(context).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL }
        tickerHead.addView(TextView(context).apply { text = p.ticker; textSize = 20f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE) })
        tickerHead.addView(companyInfoButton(host, p.ticker), LinearLayout.LayoutParams(host.dp(24), host.dp(24)).apply { setMargins(host.dp(7), 0, 0, 0) })
        top.addView(LinearLayout(context).apply { orientation = LinearLayout.VERTICAL; addView(tickerHead); addView(TextView(context).apply { text = "${p.company} • ${shares(p.shares)} shares • entry ${money(p.avgCost)}"; textSize = 10f; setTextColor(Color.rgb(155, 166, 188)); setPadding(0, host.dp(2), 0, 0) }) }, LinearLayout.LayoutParams(0, -2, 1f))
        ...
        val buttons = LinearLayout(context).apply { orientation = LinearLayout.HORIZONTAL; setPadding(host.dp(34), host.dp(9), 0, 0) }; buttons.addView(btn("EDIT", Color.rgb(120, 200, 255)) { editPositionDialog(p) }, LinearLayout.LayoutParams(0, host.dp(58), 1f).apply { setMargins(0, 0, host.dp(4), 0) }); buttons.addView(btn("SELL SHARES", Color.rgb(255, 205, 65)) { partialSell(p, forecast) }, LinearLayout.LayoutParams(0, host.dp(58), 1f).apply { setMargins(host.dp(4), 0, host.dp(4), 0) }); buttons.addView(btn("FULL SELL", Color.rgb(255, 80, 105)) { fullSell(p, forecast) }, LinearLayout.LayoutParams(0, host.dp(58), 1f).apply { setMargins(host.dp(4), 0, 0, 0) }); c.addView(buttons)
